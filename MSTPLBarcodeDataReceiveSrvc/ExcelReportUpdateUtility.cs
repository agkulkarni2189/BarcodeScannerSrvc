...
namespace MSTPLBarcodeDataReceiveSrvc
{
    public class ExcelReportUpdateUtility : MarshalByRefObject
    {
        ...
        private string ExcelFileSystemIP;
        private Timer t;
        ...
        public ExcelReportUpdateUtility()
        {
            try
            {
                ...
                FailedEditTransactions = new Queue<BarcodeTransaction>();
                t = new Timer(10000);
        ...
        public void SetClientSystemIP(string DeviceIP)
        {
            this.ExcelFileSystemIP = DeviceIP;
        }

        public void CreateExcelFileInRemoteServerIfNotExists()
        {
            try
            {
                if (!File.Exists("\\\\" + ExcelFileSystemIP + "\\" + ConfigurationManager.AppSettings.Get("ExcelWorksheetDirLoc") + ConfigurationManager.AppSettings.Get("ExcelFileName") + ".xlsx"))
                {
                    using (BarcodeExcelPkg = new ExcelPackage(new FileInfo("\\\\" + ExcelFileSystemIP + "\\" + ConfigurationManager.AppSettings.Get("ExcelWorksheetDirLoc") + ConfigurationManager.AppSettings.Get("ExcelFileName") + ".xlsx")))
                    {
                        ...
                        File.WriteAllBytes("\\\\" + ExcelFileSystemIP + "\\" + ConfigurationManager.AppSettings.Get("ExcelWorksheetDirLoc") + ConfigurationManager.AppSettings.Get("ExcelFileName") + ".xlsx", BarcodeExcelPkg.GetAsByteArray());
                        logger.Info("Excel file created successfully at: \\" + ExcelFileSystemIP + "\\" + ConfigurationManager.AppSettings.Get("ExcelWorksheetDirLoc") + " With name: " + ConfigurationManager.AppSettings.Get("ExcelFileName") + ".xlsx");
                    }
                }

                if (FailedTransactionUpdateTask.Status != TaskStatus.Running)
                {
        ...
        public void UpdateExcel(BarcodeTransaction bt)
        {
            try
            {
                CreateExcelFileInRemoteServerIfNotExists();

                using (var XlPkg = new ExcelPackage(new FileInfo("\\\\" + ExcelFileSystemIP + "\\" + ConfigurationManager.AppSettings.Get("ExcelWorksheetDirLoc") + ConfigurationManager.AppSettings.Get("ExcelFileName") + ".xlsx")))
                {
                    ...
                    File.WriteAllBytes("\\\\" + ExcelFileSystemIP + "\\" + ConfigurationManager.AppSettings.Get("ExcelWorksheetDirLoc") + ConfigurationManager.AppSettings.Get("ExcelFileName") + ".xlsx", XlPkg.GetAsByteArray());

                    logger.Info("Excel report updated successfully at system: " + ExcelFileSystemIP);
                }
            }
        ...
        public void EditExcelEntries(string BarcodeTransID = "", string ModSerNum = "", string ReaderSerialNum = "", string LaminatorNum = "")
        {
            ...
            {
                if (!File.Exists("\\\\" + ExcelFileSystemIP + "\\" + ConfigurationManager.AppSettings.Get("ExcelWorksheetDirLoc") + ConfigurationManager.AppSettings.Get("ExcelFileName") + ".xlsx"))
                    throw new FileNotFoundException("Can not find specified file at destination server on the given path");

                if (Int64.Parse(BarcodeTransID) > 0)
                {
                    using (ExcelPackage ep = new ExcelPackage(new FileInfo("\\\\" + ExcelFileSystemIP + "\\" + ConfigurationManager.AppSettings.Get("ExcelWorksheetDirLoc") + ConfigurationManager.AppSettings.Get("ExcelFileName") + ".xlsx")))
                    {
                        ...
                        File.WriteAllBytes("\\\\" + ExcelFileSystemIP + "\\" + ConfigurationManager.AppSettings.Get("ExcelWorksheetDirLoc") + ConfigurationManager.AppSettings.Get("ExcelFileName") + ".xlsx", ep.GetAsByteArray());
            ...
            {
                logger.Error("File not found exception has occured on EditExcelEntries(): " + fnfe.Message);
                throw fnfe;
            }
            catch (Exception ex)
            {
