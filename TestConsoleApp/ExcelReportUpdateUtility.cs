    ...
    public class ExcelReportUpdateUtility : MarshalByRefObject
    {
        ...
        public ExcelReportUpdateUtility()
        {
            try
            {
                ...
                //fs = new FileStream(ConfigurationManager.AppSettings.Get("ExcelWorksheetDirLoc") + ConfigurationManager.AppSettings.Get("ExcelFileName") + ".xlsx", FileMode.OpenOrCreate, FileAccess.Read);

                //foreach (DataRow dr in ExcelDataTable.Rows)
                //{
                //    logger.Info(dr[0].ToString() + "\t");
                //    logger.Info(dr[1].ToString() + "\t");
                //    logger.Info(dr[2].ToString() + "\t");
                //    logger.Info(dr[3].ToString() + "\t");
                //    logger.Info(dr[4].ToString() + "\t");
                //    logger.Info(dr[5].ToString() + "\t");
                //    logger.Info("\n");
                //}
            }
